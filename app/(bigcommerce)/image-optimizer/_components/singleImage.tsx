...
export default function Home({ image, checkbox, setUpdatedAltText, componentKey, setCheckedImage, openSettingsModal }: { image: any, checkbox: any, setUpdatedAltText: any, componentKey: any, setCheckedImage: any, openSettingsModal?: () => void}) {
  ...
  return (<>
    ...
    <Previewmodal show={previewModal} onHide={() => setPreviewModal(false)} image={image} RestoreOptimizeImage={RestoreOptimizeImage} size={size} />
    <div className="optimizerList-box d-flex align-item-center gap-3">
      ...
      <div className="optimizerProduct-imgfeild flex-grow-1 d-flex align-item-center gap-3">
        <div className="custom-input link-iconDropi flex-grow-1">
          <span>
            File Name
            <a href={`${homeUrl}/${image.url}`} target="_blank"><Image src={`${basePath}/images/link-icon.svg`} width={20} height={20} alt="" /></a>
          </span>
          <input type="text" placeholder="File name" className="form-control" value={image.image_file.split('/').pop()} disabled />
        </div>

        <div className="custom-input flex-grow-1">
          <span>Alt Text</span>
          <input disabled={image.image_file == ''} type="text" placeholder="Alt Text" className="form-control" value={altText} onChange={(e) => {
      ...
      <div className="optimizerProduct-actionInfo d-flex align-item-center gap-3">
        ...
        {optimizeStatus == 5 ?
          ...
          optimizeStatus == 1 ?
            <>
              <button type="button" className="imageOptimize-btn custom-btn" onClick={() => setPreviewModal(true)}>Preview</button>
              <OverlayTrigger
                placement="top"
                overlay={<Tooltip>Restore</Tooltip>}>
            ...
            optimizeStatus == 2 ?
              <>
                <button type="button" className="imageOptimize-btn btn btn-default btn-disable">Optimizing</button>
                <OverlayTrigger
                  placement="top"
                  overlay={<Tooltip>Restore</Tooltip>}>
                  <button type="button" className="custom-btn black-iconBtn btn-disable">
              ...
              optimizeStatus == 3 ?
                <>
                  <button type="button" className="imageOptimize-btn btn btn-default btn-disable">Restoring</button>
                  <OverlayTrigger
                    placement="top"
                    overlay={<Tooltip>Restore</Tooltip>}>
                    <button type="button" className="custom-btn black-iconBtn btn-disable">
                ...
                <>
                  <button disabled={image.image_file == ''} type="button" className={`btn btn-default ${image.image_file == '' && 'btn-disable'}`} onClick={imageOptimize}>Optimize</button>
                  <OverlayTrigger
                    placement="top"
                    overlay={<Tooltip>Restore</Tooltip>}>
                    <button disabled={image.image_file == ''} type="button" className="custom-btn black-iconBtn btn-disable" >
                      <Image src={`${basePath}/images/restore-icon.svg`} width={20} height={20} alt="" />
                    </button>
                  </OverlayTrigger>
                </>}
