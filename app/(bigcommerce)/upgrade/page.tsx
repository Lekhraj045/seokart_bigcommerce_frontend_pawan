...
export default function Home() {
  ...
  const productArray = [
    1000, 5000, 10000, 25000, 50000, 100000, 500000, 1000000, 2000000,
  ];
  ...
  const getCurrentPlan = () => {
    Api("payment/getCurrentPlan").then(({ data }) => {
      ...
      if (data.proPlanData.plan_id) {
        ...
        setProduct(
          productArray.find((item: any) => item > data.total_live_product),
        );
      }
    });
  };
  ...
  const getPlanPrice = () => {
    setLoading(true);
    Api("payment/getPlanPrice", { product: product, keyword: keyword }).then(
      ({ data }) => {
        setLoading(false);
        setPlanId(data.id);
            ...
            <div className="py-3 lg:py-6 flex justify-between items-start md:items-center gap-3 flex-col md:flex-row">
              <div className="content-frameHead-left flex flex-col">
                <div className="flex flex-col">
                  ...
                  <p className="text-xs text-[#616161] mt-0.5">
                    Choose a plan and adjust deliverables. Totals update
                    instantly based on your selections.
                  </p>
                </div>
              </div>
              ...
              <div className="invoiceLink-mobile">
                <Link href="/invoices">
                  <button
                    type="button"
                    className="btn-primary white-iconBtn d-flex align-item-center gap-1"
                  >
                    Invoices
                  </button>
                </Link>
