...
export default function Home(Props: any) {
  ...
  useEffect(() => {
    setKeyword(Props?.targetKeyword?.data ?? "");
  }, [Props?.targetKeyword]);
  return (
    <>
      <Modal
      ...
      >
        <Modal.Header closeButton={true}>
          <h1>Add Keyword to Rank Tracker</h1>
        </Modal.Header>
        <Modal.Body>
          <div className="keyword-suggestionModal">
            <div className="row">
              ...
              <div className="full-btn">
                <button className="custom-btn" onClick={addKeyword}>
                  {addKeywordBtnLoading ? <Spinner size="sm" /> : "add"}
                </button>
              </div>
            </div>
          </div>
