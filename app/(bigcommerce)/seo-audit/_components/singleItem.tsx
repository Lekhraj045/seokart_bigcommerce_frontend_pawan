  ...
  return (<>
    ...
    <tr>
            ...
            {(() => {
              ...
              if (Props.item.item_type === 'product') {
                ...
                if (isPublished !== undefined && isPublished !== null) {
                  ...
                  const isPublishedValue = isPublished === 1 || isPublished === true || isPublished === '1' || isPublished === 'published';
                  return <div className={`w-2.5 h-2.5 max-w-2.5 max-h-2.5 min-w-2.5 min-h-2.5 rounded-full ${isPublishedValue ? 'bg-[#affebf]' : 'bg-[#fed1d7]'}`}></div>;
                }
              ...
              // For non-products, show white circle
                ...
                return <div className="w-2.5 h-2.5 max-w-2.5 max-h-2.5 min-w-2.5 min-h-2.5 rounded-full bg-white"></div>;
              }
              return null;
            })()}
            {Props.item.item_name}
          </p>
          <a href={`${homeUrl}${Props.item.item_type != 'home' ? Props.item.item_url : ''}`} target="_blank"><img src="images/link-icon.svg" alt="" /></a>
        </div>
      </td>
      ...
      <td >
        ...
        <Link href={`/seo-audit/${Props.item.id}`}><button onClick={() => setLoading(true)} type="button" className="custom-btn">{loading ? <Spinner size="sm" /> : 'Optimize'}</button></Link>
