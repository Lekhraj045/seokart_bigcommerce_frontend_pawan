...
export default function Home() {
  ...
  const [productFilter, setProductFilter] = useState(localStorage.getItem('productFilter') ?? 'published_products')
  const retryCountRef = useRef(false)
  const retryItemRef = useRef(false)
  const retryTimeoutRef = useRef<{ count: ReturnType<typeof setTimeout> | null; item: ReturnType<typeof setTimeout> | null }>({ count: null, item: null })
  // If type parameter is present (coming from dashboard), reset contentTypeFilter to 'all'
  const [contentTypeFilter, setContentTypeFilter] = useState(
    hasTypeParam ? 'all' : (localStorage.getItem('contentTypeFilter') ?? 'all')
  )
    ...
    Api('getSeoAuditErrorCount', apiParams)
      .then(({ data }: any) => {
        if (!data && !retryCountRef.current) {
          ...
          retryTimeoutRef.current.count = setTimeout(() => getSeoAuditErrorCount(), 800);
          return;
        }
        if (!data) return;
        setErrorCount(data);
        ...
        setErrorType({
          'metaTagIssue':
            [
              { key: 'point1', value: 'Title Tag length is 40 to 60 characters', count: toNumber(data.title_tag_length) },
              { key: 'point2', value: 'Meta Description length is 120 - 160 characters', count: toNumber(data.meta_description_length) },
              { key: 'point3', value: 'Target Keyword present in the Title Tag', count: toNumber(data.target_keyword_present_title) },
              { key: 'point4', value: 'Target Keyword present in the Meta Description', count: toNumber(data.target_keyword_present_meta_description) },
              { key: 'point6', value: 'Duplicate Title Tags', count: toNumber(data.duplicate_title) },
              { key: 'point7', value: 'Duplicate Meta Descriptions', count: toNumber(data.duplicate_meta_descriptions) }
            ],
          'contentIssue':
            [
              { key: 'point5', value: 'Target Keyword present in the description', count: toNumber(data.target_keyword_present_description) },
              { key: 'point10', value: 'Lorem Ipsum content in the description', count: toNumber(data.lorem_Ipsum) },
              { key: 'point11', value: 'Spelling Errors in the description', count: toNumber(data.spelling_errors) },
            ],
          'imagesIssue':
            [
              { key: 'point8', value: 'Alt Text available in the Primary Image', count: toNumber(data.alt_text) }
            ],
          'brokenLinksIssue':
            [
              { key: 'internal_count', value: 'Internal Broken Links', count: toNumber(data.internal_count) },
              { key: 'external_count', value: 'External Broken Links', count: toNumber(data.external_count) },
              { key: 'point12', value: 'HTTP Links', count: toNumber(data.http_links) },
            ],
          'urlIssue':
            [
              { key: 'point13', value: 'URL length is less than 48 characters', count: toNumber(data.url_length) },
              { key: 'point14', value: 'Target Keyword present in the URL', count: toNumber(data.target_keyword_present_url) }
            ]
        });
      })
      .catch(() => {
    ...
    Api('getSeoAuditErrorItem', apiParams)
      .then((data: any) => {
        ...
        if (list.length === 0 && total === 0 && !retryItemRef.current) {
          ...
          retryTimeoutRef.current.item = setTimeout(() => getSeoAuditErrorItem(), 800);
        }
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (retryTimeoutRef.current.count) clearTimeout(retryTimeoutRef.current.count);
    };
  ...
  useEffect(() => {
    ...
    return () => {
      if (retryTimeoutRef.current.item) clearTimeout(retryTimeoutRef.current.item);
    };
  }, [limit, currentPage, point, productFilter, contentTypeFilter, errorHeading])
  ...
  useEffect(() => {
    if (errorHeading === 'imagesIssue' && point === 'all' && errorType && errorType[errorHeading] && errorType[errorHeading].length > 0) {
          ...
          <div className="d-flex align-item-center gap-3 seo-optimierPage-left">
            <div className="custom-dropi without-labelDropi">
              ...
              >
                ...
                <option value="published_products">🟢 Published Products</option>
                <option value="unpublished_products">🔴 Unpublished Products</option>
            ...
            <div className="custom-dropi without-labelDropi">
              <select className="form-select" aria-label="Default select example" value={errorHeading} onChange={(e) => {
                ...
                if (newErrorHeading === 'imagesIssue' && errorType[newErrorHeading] && errorType[newErrorHeading].length > 0) {
                ...
                {errorType[errorHeading] && (
                  <>
                    {errorHeading !== 'imagesIssue' && (
                      <option value="all">
                        All ({errorType[errorHeading].reduce((total: number, item: any) => total + (item.count ?? 0), 0)})
                      </option>
                    )}
                    {errorType[errorHeading].map((item: any, key: any) => (
                      <option key={key} value={item.key}>{item.value} ({item.count ?? 0})</option>
                    ))}
                  </>
                )}
              </select>
