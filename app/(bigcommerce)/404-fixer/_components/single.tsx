...
export default function Home({ item }: { item: any }) {
  const [redirectUrl, setRedirectUrl] = useState(
    item[item.length - 1].redirect_url ?? "",
  );
  ...
  const refresh = () => {
    Api("getSingleItemOptimize", {
      id: item[item.length - 1].seoaudit_id,
    }).then(() => {
        ...
        <div className={item.length > 1 ? "groupLink-inner" : ""}>
          <div className="error-fixerBox">
                ...
                <div className="optimizerProduct-imgfeild flex-grow-1 d-flex align-item-center gap-3">
                  ...
                  <div className="custom-input link-iconDropi flex-grow-1">
                    ...
                    <input
                      type="text"
                      className="form-control"
                      value={`${homeUrl}${row?.seoaudit?.url}`}
                      disabled
                    />
                  </div>
                </div>
            ...
            <div className="error-fixedRedirect d-flex align-item-center gap-3">
              ...
              <button
                type="button"
                className={`btn btn-default ${isChange == false || redirectUrl.length == 0 ? "btn-disable" : ""}`}
                onClick={updateInternalLink}
                disabled={isChange == false || redirectUrl.length == 0}
              >
                {buttonLoading ? (
                  <Spinner size="sm" />
                ) : isChange ? (
              ...
              >
                ...
                >
                  <Image
                    src={`${basePath}/images/refresh-icon.svg`}
                    width={16}
                    height={16}
                    alt=""
                  />
