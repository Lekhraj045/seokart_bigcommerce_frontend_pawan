...
const Home = (Props: any, ref: any) => {
  ...
  // Debounced version of your API call
  const debouncedGoogleKeyword = useMemo(
    () =>
  ...
  //   Api("getGoogleSuggestedKeyword", { keyword: targetKeyword.data }).then(
  //     ({ data }) => {
  //       setGoogleSuggestedKeyword(data);
  //     },
  //   );
  ...
  useEffect(() => {
    ...
    if (keyword && keyword !== prevKeyword.current) {
      debouncedGoogleKeyword(keyword); // call debounced API
  ...
  const handleRunSeoCopilot = async (checkedItems: any) => {
    try {
      const response = await copilotApi("useCredit", {});
      Props.getCredits();
    ...
    try {
      if (checkedItems.targetKeyword) {
        const keywordResponse = await copilotApi("getAiTargetKeyword", {
          item_name: name,
          item_id: itemId,
          item_db_id: id,
          type: itemType,
        ...
        setTargetKeyword({
          loading: false,
          data: keywordResponse?.data?.keyword,
        });
          ...
          copilotApi("getItemMetaDesc", {
            ...
            .then((res) => {
              if (res.success || res.status_code == 200) {
                setMetaDescription({
                  loading: false,
                  data: res?.data?.meta_description ?? "",
                });
                setIsSeoUpdated(true);
              }
            })
  ...
  return (
    <>
      ...
      />
      <div
        className="content-frame-main copilotRight-scroll p-0"
        onClick={() => {
